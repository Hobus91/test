...
Config.Priority = {
    ...
    ["steam:11000010770ed62"] = 100,
    ["ip:127.0.0.0"] = 85
}

-- easy localization
Config.Language = {
    joining = "Tentative de connection...",
    connecting = "Connection...",
    err = "Erreur: Impossible de récupérer aucun de vos identifiants, essayez de redémarrer votre Steam.",
    pos = "Vous êtes %d/%d dans la file d'attente"
}
-----------------------------------------------------------------------------------------------------------------------
...
function Queue:AddToQueue(ids, connectTime, name, src)
    ...
    for k,v in ipairs(self.QueueList) do
        if tmp.priority then
            ...
            if _pos then
                self:DebugPrint(string_format("%s[%s] est prioritaire, placement %d/%d dans la file d'attente", tmp.name, ids[1], _pos, queueCount))
                break
            end
        end
    end
    ...
    if not _pos then
        _pos = self:GetSize() + 1
        self:DebugPrint(string_format("%s[%s] est placé %d/%d dans la file d'attente", tmp.name, ids[1], _pos, queueCount))
    end
    ...
    if type(id) == "table" then
        for k, v in pairs(id) do
            if k and type(k) == "string" and v and type(v) == "number" then
                ...
                self:DebugPrint("Erreur lors de l'ajout d'un ID de priorité, les données non valides sont transmises")
                return false
            end
        end
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    local function updateDeferral(msg, letJoin)
        if letJoin then
            ...
            if not added then
                ...
                Queue:DebugPrint("Le joueur n'a pas pu être ajouté dans la liste d'attente")
                return
            end

            Queue:RemoveFromQueue(ids)
            Queue:DebugPrint(name .. "[" .. ids[1] .. "] est en chargement...")
    ...
    if not ids then
        ...
        Queue:DebugPrint("Dropped " .. name .. ", n'a pas pu récupérer l'un de leurs ID")
        return
    end
    ...
    if Queue:IsInQueue(ids) then
        Queue:RemoveFromQueue(ids)
        Queue:DebugPrint(string_format("%s[%s] a été retiré de la file d'attente, ils ont annulé la connexion et ont tenté de rejoindre", name, ids))
    end

    local reason = "Vous avez été expulsé de la file d'attente"
    ...
    Citizen.CreateThread(function()
        ...
        while i <= Queue:GetSize() do
            ...
            if not data.ids or not data.name or not data.firstconnect or data.priority == nil or not data.source then
                ...
                Queue:DebugPrint(tostring(data.name) .. "[" .. tostring(data.ids[1]) .. "] a été retiré de la file d'attente car il avait des données invalides")
            ...
            elseif GetPlayerLastMsg(data.source) >= 25000 and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                ...
                Queue:DebugPrint(data.name .. "[" .. data.ids[1] .. "] a été retiré de la file d'attente, car ils a expiré")
            else
                i = i + 1
            end
        end
        ...
        while i <= Queue:ConnectingSize() do
            ...
            if GetPlayerLastMsg(data.source) >= 25000 and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                Queue:RemoveFromQueue(data.source, true)
                Queue:RemoveFromConnecting(data.source, true)
                Queue:DebugPrint(data.name .. "[" .. data.ids[1] .. "] a été retiré de la file d'attente de connexion, car ils a expiré")
            else
                i = i + 1
            end
        end
