...
function doorInteract(i)
  if doorList[i]['job'] ~= nil and PlayerData.job.name == doorList[i]['job'] then
	...
	DisplayNotify("Vous n'avez ~r~pas ~w~ accès à cette zone.")
...
Citizen.CreateThread(function()
    while true do
      ...
      for i = 1, #doorList do
          ...
          local closeDoor = GetClosestObjectOfType(doorList[i]["x"], doorList[i]["y"], doorList[i]["z"], 4.0, hash, false, false, false)
          ...
          if(playerDistance < 1.5) then
			if doorList[i]["range"] == 1 then

              if doorList[i]["locked"] == true then
                  DrawText3d(doorList[i]["txtX"], doorList[i]["txtY"], doorList[i]["txtZ"], "[E] pour ~g~ouvrir ~w~la porte")
              else
                  DrawText3d(doorList[i]["txtX"], doorList[i]["txtY"], doorList[i]["txtZ"], "[E] pour ~r~fermer ~w~la porte")
              end
...
function DisplayNotify(title, text)
	SetNotificationTextEntry("STRING");
	AddTextComponentString(text);
	SetNotificationMessage("CHAR_BLOCKED", "CHAR_BLOCKED", true, 1, "Alerte de sécurité", title, text);
