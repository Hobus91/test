...
function OpenCloakroomMenu()
	...
	local elements = {}
	table.insert(elements, {label = _U('wine_clothes_civil'), value = 'citizen_wear'})
	table.insert(elements, {label = _U('wine_clothes_wine'), value = 'work_wear'})
...
function OpenMobilefermierActionsMenu()

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_fermier_actions',
		{
			title    = 'fermier',
			align    = 'top-left',
			elements = {
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_fermier', 'fermier', amount)
								end)
							end
						end
					end,
...
AddEventHandler('esx_fermier:hasEnteredMarker', function(zone)
	if zone == 'BleFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'fermier'  then
	...
	if zone == 'TraitementBread' and PlayerData.job ~= nil and PlayerData.job.name == 'fermier'  then
		CurrentAction     = 'bread_traitement'
	...
	if zone == 'FermierActions' and PlayerData.job ~= nil and PlayerData.job.name == 'fermier' then
		CurrentAction     = 'fermier_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'fermier' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'fermier' then
...
--		local blip = AddBlipForCoord(Config.Zones.fermierActions.Pos.x, Config.Zones.fermierActions.Pos.y, Config.Zones.fermierActions.Pos.z)
...
--		AddTextComponentString("fermier")
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'fermier' and (GetGameTimer() - GUI.Time) > 300 then
				...
				if CurrentAction == 'bread_traitement' then
					TriggerServerEvent('esx_fermier:startTransform', CurrentActionData.zone)
				end
		...
		if IsControlPressed(0,  Keys['F6']) and PlayerData.job ~= nil and PlayerData.job.name == 'fermier' and (GetGameTimer() - GUI.Time) > 150 then
			OpenMobilefermierActionsMenu()
			GUI.Time = GetGameTimer()
		end
	end
end)
...
RegisterNetEvent('NB:openMenufermier')
AddEventHandler('NB:openMenufermier', function()
	OpenMobilefermierActionsMenu()
end)