...
function OpenMobileRealestateagentActionsMenu()

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_realestateagent_actions',
		{
			title    = 'CenturyOtter',
			elements = {
					...
					function(data, menu)
						...
						if amount == nil then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_realestateagent', 'Agent Immobilier', amount)
							end
						end
					end,
				function(data, menu)
...
function OpenGetStocksMenu()
    ESX.TriggerServerCallback('esx_realestateagentjob:getStockItems', function(items)
      ...
      ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'stocks_menu',
        {
          title    = _U('realestateagent_stock'),
          align    = 'top-left',
          elements = elements
        },
        function(data, menu)
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsControlJustReleased(0, Keys['F6']) and PlayerData.job ~= nil and PlayerData.job.name == 'realestateagent' then
      OpenMobileRealestateagentActionsMenu()
  end
...
RegisterNetEvent('NB:openMenuRealestateagent')
AddEventHandler('NB:openMenuRealestateagent', function()
	OpenMobileRealestateagentActionsMenu()
end)