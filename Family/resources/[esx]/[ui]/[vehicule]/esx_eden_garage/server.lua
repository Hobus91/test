...
AddEventHandler('eden_garage:modifystate', function(vehicleProps, state, idGarage)
	...
	for _,v in pairs(vehicules) do
		if(plate == v.plate)then
			...
			MySQL.Sync.execute("UPDATE owned_vehicles SET idGarage =@idGarage WHERE id=@id",{['@idGarage'] = idGarage, ['@id'] = v.id})
...
ESX.RegisterServerCallback('eden_garage:idGarage', function(source, cb, posGarage)
	MySQL.Async.fetchAll("SELECT * FROM garage",{}, function(data) 
		for _,v in pairs(data) do
...
ESX.RegisterServerCallback('eden_garage:infoGarage', function(source, cb, idGarage)
	local garage = {}
	MySQL.Async.fetchAll("SELECT * FROM garage WHERE id=@idGarage LIMIT 1", {['idGarage'] = idGarage}, function(data)
		for _,v in pairs(data) do
