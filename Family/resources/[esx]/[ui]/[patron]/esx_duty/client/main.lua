...
AddEventHandler('esx_duty:hasEnteredMarker', function (zone)
  ...
  if zone == 'FoodtruckDuty' then
    CurrentAction     = 'foodtruck_duty'
    CurrentActionMsg  = _U('duty')
    CurrentActionData = {}
  end
  ...
  if zone == 'FermierDuty' then
    CurrentAction     = 'fermier_duty'
    CurrentActionMsg  = _U('duty')
    CurrentActionData = {}
  end
  if zone == 'BrinksDuty' then
    CurrentAction     = 'brinks_duty'
    CurrentActionMsg  = _U('duty')
    CurrentActionData = {}
  end
end)
              ...
              if CurrentAction == 'banker_duty' then
                ...
                if CurrentAction == 'fueler_duty' then
                  ...
                  if CurrentAction == 'realestateagent_duty' then
                    ...
                    if CurrentAction == 'foodtruck_duty' then
                      if PlayerData.job.name == 'foodtruck' or PlayerData.job.name == 'offfoodtruck' then
                        TriggerServerEvent('duty:foodtruck')
                      if PlayerData.job.name == 'foodtruck' then
                        sendNotification(_U('offduty'), 'success', 2500)
                        Wait(1000)
                      else
                        sendNotification(_U('onduty'), 'success', 2500)
                      ...
                      sendNotification(_U('notfood'), 'error', 5000)
                      Wait(1000)
                      end
                    end

                    if CurrentAction == 'fermier_duty' then
                      if PlayerData.job.name == 'fermier' or PlayerData.job.name == 'offfermier' then
                        TriggerServerEvent('duty:fermier')
                      if PlayerData.job.name == 'fermier' then
                        sendNotification(_U('offduty'), 'success', 2500)
                        Wait(1000)
                      else
                        sendNotification(_U('onduty'), 'success', 2500)
                      ...
                      sendNotification(_U('notferm'), 'error', 5000)
                      Wait(1000)
                      end
                    end

                    if CurrentAction == 'brinks_duty' then
                      if PlayerData.job.name == 'brinks' or PlayerData.job.name == 'offbrinks' then
                        TriggerServerEvent('duty:brinks')
                      ...
                      sendNotification(_U('notbrin'), 'error', 5000)
                      Wait(1000)
                      end
                    end
