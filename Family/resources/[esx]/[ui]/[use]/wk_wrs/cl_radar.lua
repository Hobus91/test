    ...
    if ( IsPedSittingInAnyVehicle( ped ) ) then 
        ...
        if ( GetVehicleClass( vehicle ) == 18 ) then
            ...
            if ( radarEnabled ) then 
                Notify( "~b~Radar activé." )
            else 
                Notify( "~b~Radar désactivé." )
            end 
...
function ToggleSpeedType()
    if ( radarInfo.speedType == "mph" ) then 
        ...
        Notify( "~b~Type de vitesse définie sur Km/h." )
    else 
        radarInfo.speedType = "mph"
        Notify( "~b~Type de vitesse définie sur MPH." )
    end
end 
...
function ToggleLockBeep()
    if ( radarInfo.lockBeep ) then 
        ...
        Notify( "~b~Bip de verrouillage rapide du radar désactivé." )
    else 
        radarInfo.lockBeep = true
        Notify( "~b~Bip de verrouillage rapide du radar activé." )
    end    
end 
