...
function RemoveOwnedVehicle (plate) --MIGRATE VEHICLES
...
AddEventHandler('esx_vehicleshop:setVehicleOwned', function (vehicleProps)
  ...
  MySQL.Async.execute(
    'INSERT INTO owned_vehicles (vehicle, owner, plate) VALUES (@vehicle, @owner, @plate)', --MIGRATE
    {
      ['@vehicle'] = json.encode(vehicleProps),
      ['@owner']   = xPlayer.identifier,
      ['@plate']   = vehicleProps.plate, --MIGRATE METHOD
    },
    function (rowsChanged)
      TriggerClientEvent("esx_vehicleshop:notify", _source, "CHAR_CARSITE", 1, _U('vehicle_belongs', vehicleProps.plate))
    end
  )
end)
...
AddEventHandler('esx_vehicleshop:setVehicleOwnedPlayerId', function (playerId, vehicleProps)
  ...
  MySQL.Async.execute(
    ...
    function (rowsChanged)
      TriggerClientEvent("esx_vehicleshop:notify", playerId, "CHAR_CARSITE", 1, _U('vehicle_belongs', vehicleProps.plate))
    end
  )
end)
...
AddEventHandler('esx_vehicleshop:sellVehicle', function (vehicle)
  MySQL.Async.fetchAll(
    ...
    function (result)
      ...
      MySQL.Async.execute(
        'DELETE FROM cardealer_vehicles WHERE plate = @plate', --MIGRATE
        { ['@plate'] = plate, } --MIGRATE
      )
    end
  )
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function (inventory)
		...
		if count > 0 and item.count >= count then
			...
			if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
				TriggerClientEvent("esx_vehicleshop:notify", _source, "CHAR_CARSITE", 1, _U('player_cannot_hold'))
			else
				inventory.removeItem(itemName, count)
				xPlayer.addInventoryItem(itemName, count)
        TriggerClientEvent("esx_vehicleshop:notify", _source, "CHAR_CARSITE", 1, _U('have_withdrawn', count, item.label))
        if Config.EnableJobLogs == true then
					TriggerEvent('esx_joblogs:AddInLog', 'vehicleshop', 'getSharedInventory', xPlayer.name, count, item.label)
				end
			end
		else
			TriggerClientEvent("esx_vehicleshop:notify", _source, "CHAR_CARSITE", 1, _U('not_enough_in_society'))
		end
	end)
end)
...
AddEventHandler('esx_vehicleshop:putStockItems', function (itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function (inventory)
      ...
      TriggerClientEvent("esx_vehicleshop:notify", _source, "CHAR_CARSITE", 1, _U('have_deposited', count, item.label))
      if Config.EnableJobLogs == true then
				TriggerEvent('esx_joblogs:AddInLog', 'vehicleshop', 'putStockItems', xPlayer.name, count, item.label)
			end
		else
			TriggerClientEvent("esx_vehicleshop:notify", _source, "CHAR_CARSITE", 1, _U('invalid_amount'))
		end
	end)
end)
...
ESX.RegisterServerCallback('esx_vehicleshop:buyVehicle', function (source, cb, vehicleModel)
  ...
  local taxe = vehicleData.price --AJOUT TAXE
  local societyAccount = nil --AJOUT TAXE

  TriggerEvent('esx_addonaccount:getSharedAccount', 'society_gouvernor', function(account) --AJOUT TAXE
		societyAccount = account --AJOUT TAXE
    end) --AJOUT TAXE

  local taxe = math.floor(taxe / 100 * 20) --AJOUT TAXE 

  if xPlayer.get('money') >= vehicleData.price then
    xPlayer.removeMoney(vehicleData.price)
    societyAccount.addMoney(taxe) --AJOUT TAXE
    cb(true)
  else
    cb(false)
  end
  ...
  MySQL.Async.fetchAll(
    ...
    function (result)
      if #result > 0 then
        ...
        MySQL.Async.execute(
          'DELETE FROM rented_vehicles WHERE plate = @plate', --MIGRATE
          { ['@plate'] = plate } --MIGRATE
        )
        ...
        function (result)
          ...
          for i=1, #result, 1 do
            ...
            if foundPlayer then
              ...
              TriggerClientEvent("esx_vehicleshop:notify", xPlayer.source, "CHAR_CARSITE", 1, _U('paid_rental') .. result[i].rent_price)
            else
